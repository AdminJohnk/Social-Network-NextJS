...
const buttonVariants = cva(
  'duration-500 inline-flex items-center justify-center rounded-lg text-sm font-medium disabled:pointer-events-none disabled:opacity-50 focus:outline-none',
  {
    variants: {
      variant: {
        default:
  ...
  ({ className, variant, size, asChild = false, preIcon, sufIcon, ...props }, ref) => {
    return (
      <button className={cn(buttonVariants({ variant, size, className }))} ref={ref} {...props}>
        {preIcon && <span className='mr-1 text-xl max-lg:!mr-0'>{preIcon}</span>}
        {props.children}
        {sufIcon && <span className='ml-1 text-xl max-lg:!mr-0'>{sufIcon}</span>}
      </button>
    );
  }
);
