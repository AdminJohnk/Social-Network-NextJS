...
const MessageBox = forwardRef<HTMLDivElement, IMessageBoxProps>(
  (
    ...
    const roundedCornerStyle = useMemo(() => {
      if (isOwn) {
        if (isNextMesGroup && isPrevMesGroup)
          return 'rounded-s-[1.5rem] rounded-e-[0.75rem]';
        if (isNextMesGroup && !isPrevMesGroup)
        ...
        if (isNextMesGroup && isPrevMesGroup)
          return 'rounded-e-[1.5rem] rounded-s-[0.75rem]';
        if (isNextMesGroup && !isPrevMesGroup)
        ...
        <>
          ...
          >
            ...
            <div
              className={cn(
                'relative',
                type === 'group' && !isOwn && !isPrevMesGroup && 'mt-7'
              )}
            >
              {type === 'group' && !isOwn && !isPrevMesGroup && (
    ...
    const receivePost = (content: string) => {
      const { post, isLoadingPost } =
        message.type === 'post'
          ? usePostData(message.post_id || '')
          : { post: undefined, isLoadingPost: true };
      return (
        <div className={cn('flex gap-3 items-end', type === 'group' && !isOwn && !isPrevMesGroup ? '' : '')}>
              ...
              {isLoadingPost ? (
                ...
                <div
                  ...
                  <div className='flex-start py-2'>
                    ...
                    <div className='ms-3'>{post.post_attributes.user.name}</div>
                  </div>
                  <div className='flex flex-col items-start'>
                    {post.post_attributes.images.length > 0 ? (
                      <div
                        key={post.post_attributes.images[0]}
                        className='relative max-w-fit w-72 h-7w-72 -mx-4'>
                        <Image
                          width={500}
                          height={500}
                    ...
                    <div className='w-full'>
                      <ShowContent
                        content={
                          post.post_attributes.content.length > 25
                            ? post!.post_attributes.content.slice(0, 25) + '...'
      ...
      return (
        <div className='flex gap-2 flex-row-reverse items-end'>
          <div
            ...
            {isLoadingPost ? (
              <div
                className={cn('bg-foreground-2 px-4 py-2', roundedCornerStyle)}
              >
                <div className='flex-start gap-3'>
                  <CircularProgress size={20} className='!text-text-1' />
                </div>
                  ...
                  <div className='flex items-center justify-center w-8 h-8 rounded-full bg-neutral-300'>
                    {
                      notification[message.type][
                        stateCalled(message.sender._id)
                      ]
                    }
                  </div>
                  <div className='flex flex-col mx-2'>
        ...
        return (
          ...
          >
            <div className='text-sm text-gray-500 font-semibold'>
              {isOwn ? t('You') : message.sender.name}&nbsp;
              {switchNoti(message)}
            </div>
          </div>
        );
