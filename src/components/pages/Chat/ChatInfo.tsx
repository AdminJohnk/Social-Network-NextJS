...
export default function ChatInfo({ conversationID }: IChatInfoProps) {
  ...
  const otherUser = useMemo(
    () => currentConversation?.members?.filter((member) => member._id !== currentUserInfo._id)[0],
          ...
          {isLoading ? (
            ...
            <>
              {firstFourImagesWithInfo.map((image, index) => (
                <div className='flex-between mb-2 ml-2' key={index}>
  ...
  const friends = currentUserInfo.members || [];

  // filter members in friends but not in conversation
  const members = useMemo(
    () =>
      friends.filter((friend) => !currentConversation?.members.some((member) => member._id === friend._id)),
  ...
  const isChangedName = groupName === currentConversation?.name;
  ...
  const isChangedAvatar = !fileAvatar;
              ...
              <>
                ...
                {messagesImage.map((item) =>
                  item.images?.map((image, index) => (
                    <div className='fileContent flex-between mb-2 ml-2' key={index}>
                      <div className='left flex-between'>
                        <div className='image mr-2 flex rounded-xl h-14 w-14 overflow-hidden'>
                          <Image
