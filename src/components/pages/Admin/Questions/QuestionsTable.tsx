...
import { useGetAllQuestionsAdmin, useGetNumberOfQuestionsAdmin } from '@/hooks/query';
...
export function QuestionsTable() {
  ...
  const { numberOfQuestions } = useGetNumberOfQuestionsAdmin();
  const { allQuestions, isFetchingAllQuestions, isLoadingAllQuestions } = useGetAllQuestionsAdmin(
    pagination.pageIndex,
    pagination.pageSize
  );

  const defaultData = useMemo(() => [] as IQuestion[], []);

  const table = useReactTable({
    data: allQuestions ?? defaultData,
    columns: questionColumns,
    rowCount: numberOfQuestions ?? 0,
    state: {
      sorting,
      columnVisibility,
      rowSelection,
          ...
          <Table /* style={{ width: table.getCenterTotalSize() }} */>
            ...
            <TableBody>
              {Array.from({ length: 10 }).map((_, index) => (
                <TableRow key={index}>
                  {Array.from({ length: questionColumns.length }).map((_, index) => (
                    <TableCell key={index}>
