...
import { useGetAllPostsAdmin, useGetNumberOfPostsAdmin } from '@/hooks/query';
...
export function PostsTable() {
  ...
  const { numberOfPosts } = useGetNumberOfPostsAdmin();
  const { allPosts, isFetchingAllPosts, isLoadingAllPosts } = useGetAllPostsAdmin(
    pagination.pageIndex,
    pagination.pageSize
  );

  const defaultData = useMemo(() => [] as IPost[], []);

  const table = useReactTable({
    data: allPosts ?? defaultData,
    columns: postColumns,
    rowCount: numberOfPosts ?? 0,
    state: {
      sorting,
      columnVisibility,
      rowSelection,
          ...
          <Table /* style={{ width: table.getCenterTotalSize() }} */>
            ...
            <TableBody>
              {Array.from({ length: 10 }).map((_, index) => (
                <TableRow key={index}>
                  {Array.from({ length: postColumns.length }).map((_, index) => (
                    <TableCell key={index}>
